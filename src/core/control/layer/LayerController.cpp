...
LayerController::LayerController(Control *control)
    : control(control), selectedPage(npos) {}
...
void LayerController::insertLayer(PageRef page, Layer *layer,
...
auto LayerController::actionPerformed(ActionType type) -> bool {
    ...
    case ACTION_RENAME_LAYER: {
        RenameLayerDialog dialog(control->getGladeSearchPath(),
...
void LayerController::addNewLayer() {
    ...
    control->getUndoRedoHandler()->addUndoAction(
        std::make_unique<InsertLayerUndoAction>(this, p, l, layerPos));
...
void LayerController::deleteCurrentLayer() {
    ...
    control->getUndoRedoHandler()->addUndoAction(
        std::make_unique<RemoveLayerUndoAction>(this, p, l, lId - 1));
...
void LayerController::moveCurrentLayer(bool up) {
    ...
    control->getUndoRedoHandler()->addUndoAction(
        std::make_unique<MoveLayerUndoAction>(this, p, currentLayer, lId - 1,
                                              newIndex));
...
void LayerController::mergeCurrentLayerDown() {
    ...
    UndoActionPtr undo_redo_action = std::make_unique<MergeLayerDownUndoAction>(
        this, page, currentLayer, layerBelow, layerID - 1, pageID);
...
void LayerController::copyCurrentLayer() {
    ...
    control->getUndoRedoHandler()->addUndoAction(
        std::make_unique<InsertLayerUndoAction>(this, p, cloned, lId));

    fireRebuildLayerMenu();
}

auto LayerController::getCurrentPage() const -> PageRef {
